...
def get_loss_train(model, trainloader, criterion, device):
    ...
    for batch, (inputs, labels) in enumerate(trainloader):
        with torch.no_grad():
            ...
            #########################################
            outputs = np.transpose(outputs.to(device), (0,2,3,1))
            preds = torch.argmax(outputs, dim=3).float()
            acc = accuracy_check_for_batch(labels.to(device), preds.to(device), inputs.size()[0])
            total_acc += acc
            total_loss += loss.to(device).item()
...
def val_model(model, valloader, criterion, device, dir):
    ...
    for batch, (inputs, labels) in enumerate(valloader):
        with torch.no_grad():
            ...
            acc = accuracy_check_for_batch(labels.to(device), preds.to(device), inputs.size()[0])
            total_val_acc += acc
            total_val_loss += loss.to(device).item()

            for i in range(preds.shape[0]):
                temp = preds[i].to(device).data.numpy()
                temp_l = labels[i].to(device).data.numpy()
